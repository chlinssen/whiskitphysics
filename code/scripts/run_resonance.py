...
n_segments = 20 # XXX: not implemented yet
...
fmin = 10.	# [Hz]
fmax = 150.		# [Hz]
n_frequency_points = 48



def write_config(dt: float, sim_time: float):
...
def plot_resonance_curve(freq, base_ampl, ampl,  fname_snip="", ax_ampl=None, ax_phase=None, label=None, colour=None):
	if colour is None:
		colour = "blue"
	fig = None
	if not ax_ampl:
...
for fidx in range(len(freq)):
	...
	ampl_base, ampl_tip = read_data('../output/resonance/kinematics/x/RC1.csv', n_segments=n_segments)
	...
	plot_timeseries(timevec, tidx_analysis_start, tidx_analysis_stop, ampl_base, ampl_tip, ampl_tip_mag, "/tmp/test_[f=" + str(f) + " Hz].png")
	plot_resonance_curve(freq, ampl_base, measured_ampl)
